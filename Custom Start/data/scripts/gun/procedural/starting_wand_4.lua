...
local is_wand_4_enabled = ModSettingGet("Custom_Start.enable_quaternary_wand") or false

-- Retrieve the player's position
local entity_id = GetUpdatedEntityID()
local x, y = EntityGetTransform(entity_id)
SetRandomSeed(x, y - 12)

-- If the toggle is not enabled, kill the entity (remove the spawned wand)
if not is_wand_4_enabled then
    print("Debug: Quaternary wand is disabled, deleting entity.")
    EntityKill(entity_id)
    return
end

-- Set the position of the wand
local new_x, new_y = 635, -95
...
print("Debug: Selected sprite for Quaternary Wand: " .. random_sprite)

-- Create the wand properties
local ability_comp = EntityGetFirstComponent(entity_id, "AbilityComponent")

local gun = {
    name = {"Quaternary Wand"},
    actions_per_round = 1,
    reload_time = tonumber(ModSettingGet("Custom_Start.quaternary_cast_delay") or 20), -- Adjusted for cast delay
    shuffle_deck_when_empty = 0,
    fire_rate_wait = tonumber(ModSettingGet("Custom_Start.quaternary_recharge_time") or 15), -- Adjusted for recharge time
    spread_degrees = 0,
    speed_multiplier = 1,
    mana_charge_speed = tonumber(ModSettingGet("Custom_Start.quaternary_mana_charge_speed") or 20),
    mana_max = tonumber(ModSettingGet("Custom_Start.quaternary_mana_max") or 100)
}

-- Set the wand's properties
if ability_comp then
    ComponentSetValue(ability_comp, "ui_name", "Quaternary Wand")
    ...
    print("Debug: AbilityComponent not found for Quaternary Wand.")
end
...
if sprite_component then
    ...
    if sprite_component then
        print("Debug: Added and applied SpriteComponent to Quaternary Wand with sprite: " .. random_sprite)
    else
        print("Debug: Failed to add SpriteComponent to Quaternary Wand.")
    end
end
...
local action_count = math.min(
    tonumber(ModSettingGet("Custom_Start.quaternary_spell_count") or 1),
    tonumber(ModSettingGet("Custom_Start.quaternary_deck_capacity") or 1)
)
local selected_spell_id = ModSettingGet("Custom_Start.quaternary_spell") or "1"
...
print("Debug: Selected spell for Quaternary Wand: " .. tostring(selected_spell))
